export function square(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, stage: number, isContentMode: boolean = false) {
  const width = innerWidth * 0.8 * (1 - 0.2 * stage);
  const height = width  * (1 - 0.2 * stage);
  ...
  ctx.lineWidth = 5;
  ctx.strokeStyle = isContentMode ? '#3131316F' : 'rgba(255, 0, 255, 0.5)';
  ctx.stroke();
